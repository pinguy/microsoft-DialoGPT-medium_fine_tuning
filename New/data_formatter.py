...
class Config:
    ...
    input_qa_dataset_path: str = 'dataset_qa_train.jsonl.gz'
...
# ============================================================================
# TEXT UTILITIES (from data_formatter.py)
# ============================================================================
...
# ============================================================================
# SEMANTIC MEMORY (from data_formatter.py)
# ============================================================================
...
# ============================================================================
# IPF SEMANTIC ENHANCER (from data_formatter.py - keep full implementation)
# ============================================================================
...
# ============================================================================
# SEMANTIC LABELER (from data_formatter.py - keep full implementation)
# ============================================================================
...
# ============================================================================
# QUALITY SCORER (from data_formatter.py)
# ============================================================================
...
# ============================================================================
# Q&A BUILDER (from data_formatter.py)
# ============================================================================
...
# ============================================================================
# OPTIMIZED DATA PROCESSOR (MERGED VERSION)
# ============================================================================
...
class OptimizedDataProcessor:
    def __init__(self, config: Config, use_semantic_filtering: bool = True):
        ...
        # Initialize components from data_formatter.py
    ...
    def load_qa_dataset(self) -> List[Dict]:
        """Load the Q&A dataset generated by adaptive_semantic.py (from data_formatter2_fixed)"""
        path = Path(self.config.input_qa_dataset_path)
        
        if not path.exists():
            logger.warning(f"Q&A dataset not found: {path}")
            logger.info("Checking for alternative locations...")
            
            alternatives = [
                'dataset_qa_train.jsonl.gz',
                'dataset_qa_train.jsonl',
                'dataset_knowledge_train.jsonl.gz',
                'dataset_knowledge_train.jsonl'
            ]
            ...
            for alt in alternatives:
                ...
                logger.warning("No Q&A dataset found. Skipping this source.")
                return []
        
        logger.info(f"Loading Q&A dataset from: {path}")
        entries = []
        ...
        try:
            ...
            logger.info(f"✓ Loaded {len(entries)} Q&A entries from {path}")
            return entries
        
        except Exception as e:
            logger.error(f"Failed to load Q&A dataset: {e}")
            return []

    def load_memory_texts(self) -> List[Dict]:
        """Load memory texts and metadata (from data_formatter.py)"""
    ...
    # ========================================================================
    # PARSE AND FIX ENTRIES (from data_formatter2_fixed)
    # ========================================================================

    def parse_and_fix_qa_entry(self, entry: Dict) -> Optional[Dict]:
        """
        Parse and fix a single Q&A entry from adaptive_semantic.py output.
        
        FIXES:
        1. Extract "user" and "assistant" from the nested structure
        2. Remove "Context: Assistant:" and "User:" prefixes from word counts
        3. Populate themes from phrase_themes if available
        """
        
        user_text = entry.get('user', '')
        assistant_text = entry.get('assistant', '')
        
        # Fix 1: Extract actual user message (remove Context and User: prefix)
        user_match = re.search(r'User:\s*(.+)', user_text, re.DOTALL)
        if user_match:
            actual_user_text = user_match.group(1).strip()
        else:
            actual_user_text = user_text
        
        # Clean the texts
        actual_user_text = clean_text(actual_user_text)
        assistant_text = clean_text(assistant_text)
        
        # Validate
        if not validate_text(actual_user_text, self.config) or not validate_text(assistant_text, self.config):
            return None
        
        # Fix 2: Calculate proper word counts (without labels)
        user_words = len(actual_user_text.split())
        assistant_words = len(assistant_text.split())
        
        # Score quality
        quality_scores = self.qual_scorer.score(assistant_text)
        
        # Label semantics
        semantic_labels = {}
        if self.labeler:
            semantic_labels = self.labeler.label(assistant_text)
        
        # Fix 3: Get themes from phrase_themes
        source_metadata = entry.get('source_metadata', {})
        themes = source_metadata.get('phrase_themes', [])
        if not themes:
            themes = source_metadata.get('themes', [])
        if not themes:
            themes = semantic_labels.get('themes', [])
        
        # Create fixed entry with CORRECTED ORDER: User first, then Assistant
        fixed_entry = {
            'text': f"<|user|>{actual_user_text}<|assistant|>{assistant_text}<|endoftext|>",
            'user': actual_user_text,
            'assistant': assistant_text,
            'quality_metrics': {
                'user_len': len(actual_user_text),
                'assistant_len': len(assistant_text),
                'user_words': user_words,
                'assistant_words': assistant_words,
                'semantic_similarity': entry.get('quality_metrics', {}).get('semantic_similarity', 0.5),
                'length_ratio': len(assistant_text) / max(len(actual_user_text), 1),
                'quality_score': quality_scores.get('composite_quality', 0.5),
                'readability_score': 1.0,
                'information_density': quality_scores.get('information_density', 0.6),
            },
            'source_metadata': {
                **source_metadata,
                'themes': themes,
                'phrase_themes': semantic_labels.get('phrase_themes', []),
                'word_themes': semantic_labels.get('word_themes', []),
                'primary_theme': semantic_labels.get('primary_theme', 'general'),
                'source': 'qa_dataset'
            }
        }
    ...
    # ========================================================================
    # DEDUPLICATE AND CLEAN (from data_formatter.py)
    # ========================================================================
    ...
    # ========================================================================
    # CREATE CONVERSATIONAL PAIRS (from data_formatter.py)
    # ========================================================================
        ...
        for convo_id, msgs in tqdm(conversations.items(), desc="Creating conversational pairs"):
            ...
            for msg in msgs:
                ...
                if author == 'user':
                    ...
                    # Extract ONLY the final user message (without context/labels)
                    user_text_raw = current_user_msg['cleaned_text']
                    
                    # Try to extract just the final user message if it has prefixes
                    # Pattern 1: "Context: ... User: actual_message"
                    context_match = re.search(r'(?:Context:.*?)?User:\s*(.+)', user_text_raw, re.DOTALL | re.IGNORECASE)
                    if context_match:
                        final_user_text = context_match.group(1).strip()
                    else:
                        # Pattern 2: Just "User: message"
                        user_match = re.search(r'^User:\s*(.+)', user_text_raw, re.DOTALL | re.IGNORECASE)
                        if user_match:
                            final_user_text = user_match.group(1).strip()
                        else:
                            # No prefix found, use the text as-is
                            final_user_text = user_text_raw
                    
                    # Skip if the extracted text is too short or empty
                    if not final_user_text or len(final_user_text.strip()) < 3:
                        logger.debug(f"Skipping empty user message in conversation {convo_id}")
                        current_user_msg = None
                        continue
    ...
    # ========================================================================
    # CREATE PDF Q&A PAIRS (from data_formatter.py)
    # ========================================================================
...
def parse_args() -> Config:
    ...
    p = argparse.ArgumentParser(description='Merged Data Formatter - Combines Q&A Dataset + Memory Texts')
    
    # --- IO and General ---
    p.add_argument('--input-qa-dataset', default='dataset_qa_train.jsonl.gz', 
                   help='Input Q&A dataset from adaptive_semantic.py')
    ...
    p.add_argument('--semantic-mode', choices=['normal', 'adaptive'], default='adaptive', help='Semantic mode')
    ...
    cfg = Config(
        input_qa_dataset_path=args.input_qa_dataset,
...
def main(cfg: Config):
    """Main processing pipeline - MERGED VERSION"""
    
    logger.info("=" * 70)
    logger.info("MERGED DATA FORMATTER - Q&A Dataset + Memory Texts")
    logger.info("=" * 70)
    logger.info(f"Q&A Dataset: {cfg.input_qa_dataset_path}")
    ...
    # Load Q&A dataset from adaptive_semantic.py
    qa_entries_raw = processor.load_qa_dataset()
    
    # Load memory texts and metadata
    memory_entries_raw = processor.load_memory_texts()
    
    total_raw = len(qa_entries_raw) + len(memory_entries_raw)
    logger.info(f"\n✓ Total raw entries loaded: {total_raw}")
    logger.info(f"  - Q&A entries: {len(qa_entries_raw)}")
    logger.info(f"  - Memory entries: {len(memory_entries_raw)}")
    ...
    # ========================================================================
    # PROCESS Q&A ENTRIES (Fix format issues)
    # ========================================================================
    
    logger.info("\n🔧 PROCESSING Q&A ENTRIES...")
    fixed_qa_entries = []
    for entry in tqdm(qa_entries_raw, desc="Fixing Q&A entries"):
        fixed = processor.parse_and_fix_qa_entry(entry)
        if fixed:
            fixed_qa_entries.append(fixed)
    
    logger.info(f"✓ Fixed {len(fixed_qa_entries)} Q&A entries (filtered {len(qa_entries_raw) - len(fixed_qa_entries)} invalid)")
    ...
    all_final_pairs = fixed_qa_entries + conversational_pairs + pdf_qa_pairs
    
    logger.info(f"✓ Total merged pairs: {len(all_final_pairs)}")
    logger.info(f"  - From Q&A dataset: {len(fixed_qa_entries)}")
    ...
    logger.info(f"\nData Sources:")
    logger.info(f"  - Q&A dataset pairs: {len(fixed_qa_entries)}")
