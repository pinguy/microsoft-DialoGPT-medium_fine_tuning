# Enhanced UCS (Unified Cognition System) v3 - Production Ready
# Major improvements:
...
# - Smoke:    python UCS_v3.py --mode smoke
# - API:      RUN_API=1 UCS_REQUIRE_AUTH=0 python UCS_v3.py
# - API auth: RUN_API=1 UCS_API_KEY="secret" python UCS_v3.py
# -----------------------------------------------------------------------------
...
def softmax(x):
    if not HAS_NUMPY:
        return [1.0 / len(x)] * len(x)  # Uniform for dummy
    x = np.array(x)
...
class AsyncExpertManager:
    """Manages experts and their proposals with async support."""
    ...
    async def propose(self, ctx: Dict[str, Any]) -> List[ExpertProposal]:
        """Gathers proposals from experts across three phases: map, propose, filter."""
        # 1) map phase: allow experts to modify the context
        map_tasks = [self._call_handler_async(e["handler"], ctx) for n, e in self.experts.items() if e["phase"] == "map"]
        mapped_ctxs = await asyncio.gather(*map_tasks, return_exceptions=True)
        for m in mapped_ctxs:
            if not isinstance(m, Exception) and m is not None:
                ctx = m  # Simple overwrite; could merge

        # 2) propose phase: experts generate proposals
        proposals = []
        expert_names_propose = [name for name, e in self.experts.items() if e["phase"] == "propose"]
        propose_tasks = [self._call_handler_async(e["handler"], ctx) for name, e in self.experts.items() if e["phase"] == "propose"]
        proposal_contents = await asyncio.gather(*propose_tasks, return_exceptions=True)
        for name, proposal_content in zip(expert_names_propose, proposal_contents):
            if not isinstance(proposal_content, Exception) and proposal_content is not None:
        ...
        for n, e in self.experts.items():
            if e["phase"] == "filter":
                proposals = await self._call_handler_async(e["handler"], {"ctx": ctx, "proposals": proposals, "parent_system": self.parent_system}) or proposals

        return proposals

    async def _call_handler_async(self, handler, arg):
        try:
            result = handler(arg)
    ...
    def score_proposals(self, proposals: List[ExpertProposal], ctx: Dict[str, Any]) -> List[ExpertProposal]:
        ...
        attention_scores = [p.trust_score for p in proposals]
        if HAS_NUMPY and attention_scores:
            attn_probs = softmax(attention_scores)  # Focus on top
            for p, prob in zip(proposals, attn_probs):
                p.trust_score *= prob  # Gate
        return sorted(proposals, key=lambda p: p.trust_score, reverse=True)[:3]  # Bottleneck top 3
...
class VectorMemory:
    """Enhanced vector memory with HNSW index for real ANN performance."""
    ...
    def _rebuild_hnsw_from_scratch(self):
        ...
        try:
            _logger.info("Rebuilding HNSW index from scratch...")
...
class MetaCognitiveMonitor(threading.Thread):
    def __init__(self, system: UnifiedCognitionSystem):
        super().__init__(daemon=True)
        self.system = system
        self.metrics_history = deque(maxlen=100)  # Rolling window

    def run(self):
        while True:
            time.sleep(60)  # Monitor every minute
            metrics = {
                'cache_hit_rate': self.system.vmem.query_cache.stats()['hit_rate'],
                'expert_drifts': {name: rep.ema_reward for name, rep in self.system._reputations.items()},
                'retrieval_latency': np.mean(self.system.telemetry['retrieval_times']) if 'retrieval_times' in self.system.telemetry else 0
            }
            self.metrics_history.append(metrics)
            self._tune_system(metrics)

    def _tune_system(self, metrics):
        if metrics['cache_hit_rate'] < 0.5:
            _logger.info("Low cache hits: Increasing TTL")
            # Dynamically adjust cache TTL or other params
        for expert, drift in metrics['expert_drifts'].items():
            if drift < 0.3:
                _logger.warning(f"Disabling underperformer: {expert}")
                # self.system.expert_manager.unregister_expert(expert)  # Implement if needed
...
class UnifiedCognitionSystem:
    ...
    def __init__(self, mem: Optional[VectorMemory] = None, dim: int = DIM, 
        ...
        self.expert_manager = AsyncExpertManager(parent_system=self)
        ...
        self.meta_monitor = MetaCognitiveMonitor(self)
    ...
    async def run_async(self, prompt: str, actions: Optional[List[str]] = None, iters: int = 5, 
        ...
        """Runs the blackboard loop asynchronously."""
        ...
        for i in range(iters):
            ...
            proposals = await self.expert_manager.propose(blackboard)
    ...
    async def dream_mode(self, duration: timedelta = timedelta(minutes=5)):
        ...
        while time.time() < start + duration.total_seconds():
            # Replay: Randomly retrieve and rehearse
            random_query = np.random.normal(size=self._dim)
            random_query /= np.linalg.norm(random_query) + 1e-12
            results = self.vmem.retrieve(random_query)
            for mid, _ in results:
                # Generate hypothetical: mutate content, re-embed
                mutated = self._quick_summarize(self.vmem.mementos.get(mid, {}).get('content', '') + " hypothetical variant")
                new_mid = f"dream_{mid}"
...
if HAS_FASTAPI and os.environ.get('RUN_API') == '1' and uvicorn:
    app = FastAPI(title="Enhanced UCS API v3", 
    ...
    async def run_blackboard(data: dict):
        ...
        result = await ucs_instance.run_async(prompt, actions)
    ...
    async def save_state(payload: dict):
        ucs_instance._ensure_memory()
        path = (payload.get("path") or "ucs_v3_state.json").strip()
    ...
    async def memory_stats():
        ...
        stats = {
            ...
            "insert_buffer_size": ucs_instance.vmem._index_queue.qsize(),
...
def run_enhanced_smoke_test():
    ...
    asyncio.run(u.dream_mode(timedelta(seconds=10)))  # Short test
    _logger.info("Dream mode executed")
    
    # Test save/load
    _logger.info("Testing save/load...")
    u.vmem.save_state("test_v3_state.json")
    loaded_mem = VectorMemory.load_state("test_v3_state.json")
    ...
    # Cleanup
    import os
    for file in ["test_v3_state.json", "test_v3_state.hnsw"]:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run UCS_v3")
    parser.add_argument("--mode", type=str, default=None, help="Mode to run: smoke")
    args = parser.parse_args()

    if HAS_FASTAPI and os.environ.get('RUN_API') == '1' and uvicorn:
        _logger.info("Starting Enhanced UCS v3 FastAPI server...")
        ...
        print("No mode specified or invalid mode. Use --mode smoke for smoke test.")
        sys.exit(1)

__all__ = [
    "VectorMemory", "UnifiedCognitionSystem", "AsyncExpertManager", "ExpertProposal",
