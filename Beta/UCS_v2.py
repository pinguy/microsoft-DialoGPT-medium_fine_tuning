...
#
# Usage:
# - Smoke:    python enhanced_ucs_v2.py
# - API:      RUN_API=1 UCS_REQUIRE_AUTH=0 python enhanced_ucs_v2.py
# - API auth: RUN_API=1 UCS_API_KEY="secret" python enhanced_ucs_v2.py
# -----------------------------------------------------------------------------
...
class VectorMemory:
    ...
    def add_memento(self, mid: str, emb: np.ndarray, tags: List[str] = None, reliability: float = 0.5, 
        ...
        with self._lock:
            ...
            # Add to insert buffer for batch HNSW updates
            if self.use_advanced_search and self.hnsw_index is not None:
                self._insert_buffer.append((mid, emb))
                
                if len(self._insert_buffer) >= self._rebuild_threshold:
                    self._batch_add_to_hnsw()
            
            return True

    def _batch_add_to_hnsw(self):
        """Batch add embeddings to HNSW index."""
        if not self._insert_buffer or not self.use_advanced_search:
            return
        
        try:
            mids_to_add = [mid for mid, _ in self._insert_buffer]
            embs_to_add = np.array([emb for _, emb in self._insert_buffer])
            ...
            _logger.info(f"Batch added {len(labels)} items to HNSW index")
        except Exception as e:
            _logger.error(f"Failed to batch add to HNSW: {e}")
            # Don't clear buffer on failure so we can retry
            if len(self._insert_buffer) > self._rebuild_threshold * 2:
    ...
    def retrieve(self, query_emb: np.ndarray, top_k: int = 5, ann_K: int = 50, 
        ...
        with self._lock:
            ...
            # Ensure HNSW index is up to date
            if use_advanced and self._insert_buffer:
                self._batch_add_to_hnsw()
...
class UnifiedCognitionSystem:
    ...
    def __init__(self, mem: Optional[VectorMemory] = None, dim: int = DIM, 
                 embed_fn=None, use_advanced_search: bool = True):
        self._dim = dim
        self.vmem = mem
